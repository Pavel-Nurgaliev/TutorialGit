    ...
    public partial class UbsBgContractFrm : UbsFormBase
    {
        ...
        private void linkAgent_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (ids != null && ids.Length > 0)
            {
                ...
                {
                    RunUbsChannel("AgentRewardAvaliable", m_paramIn, m_paramOut);
                ...
                RunUbsChannel("BGReadClientById", m_paramIn, m_paramOut);
        ...
        private void btnManualBenificiar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_arrCountry == null)
                {
                    ...
                    RunUbsChannel("GetAddressData", m_paramIn, m_paramOut);
        ...
        private void btnDelRate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                TreeNode selectedNode = trvRates.SelectedNode;
                ...
                string rateType = selectedNode.Parent.Text;

                trvRates.Nodes.Remove(selectedNode);
                ...
                if (m_arrRates != null && m_arrRateValues != null)
                {
                    ...
                    {
                        string currentRateType = Convert.ToString(m_arrRates[i]);
                        if (currentRateType != rateType)
                            continue;

                        if (m_arrRateValues[i] is object[,] arrRateTemp)
                        {
                            ...
                            for (int j = 0; j < rows; j++)
                            {
                                ...
                                if (cellDate != null &&
                                    Convert.ToDateTime(cellDate).ToShortDateString() == datePart)
                                {
                                    deleteIndex = j;
                                    break;
                                }
                            ...
                            if (deleteIndex >= 0)
                            {
                                if (rows == 1)
                                {
                                    m_arrRateValues[i] = null;
                                }
                                else
                                {
                                    var newArr = new object[rows - 1, 2];
                                    int target = 0;
                                    for (int src = 0; src < rows; src++)
                                    {
                                        if (src == deleteIndex) continue;
                                        newArr[target, 0] = arrRateTemp[src, 0];
                                        newArr[target, 1] = arrRateTemp[src, 1];
                                        target++;
                                    }
        ...
        private void trvRates_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                if (e.Node == null)
                ...
                if (node.Parent != null)
                {
                    ...
                    btnEditRate.Enabled = true;
                    btnDelRate.Enabled = true;
                }
                else
                {
        ...
        private void UbsBgContractFrm_Ubs_ActionRunBegin(object sender, UbsActionRunEventArgs args)
        {
            ...
            else if (args.Action == ActionUbsBgGuarListContract)
            {
            ...
            else if (args.Action == ActionUbsGuarOperationList)
            {
        ...
        private void InitDocForCopyCommand()
        {
            ...
            RunUbsChannel("BG_Contract_Copy", m_paramIn, m_paramOut, true);
            ...
            this.ubsCtrlFields.Collection["Очередность платежей"].Value = m_paramOut.Value("Реквизиты оплаты гарантии");
        ...
        private bool ReReadDocLoadAndValidateContract()
        {
            ...
            RunUbsChannel("BG_Contract_Read", m_paramIn, m_paramOut, true);
        ...
        private void FilllstAccounts()
        {
            try
            {
                ...
                m_paramIn.Value("Счета", m_arrAccounts);
                RunUbsChannel("BGReadAccountsByContract", m_paramIn, m_paramOut);
        ...
        private void SetInfoByFrameContract()
        {
            ...
            m_idCurrency = Convert.ToInt32(m_paramOut.Value("Идентификатор валюты рамочного договора"));
        ...
        private void SetBonusCtrlsBonusState()
        {
            ...
            if (cmbOrderPayFeeBonus.Text == OrderPayPeriodically)
            {
                if (Convert.ToInt32(cmbState.SelectedValue) == 4)
                {
        ...
        //TODO: implement lvwAccounts_KeyDown
        private void lvwAccounts_KeyDown(object sender, KeyEventArgs e)
        {
            throw new NotImplementedException();
        }

        //TODO: implement cmsAccounts_Click
        private void cmsAccounts_Click(object sender, EventArgs e)
        {
            throw new NotImplementedException();
        }
    }
