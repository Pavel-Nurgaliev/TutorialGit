...
namespace UbsBusiness
{
    ...
    {
        private const string EmptyAccountNumber = "00000000000000000000";

        #region Блок объявления переменных

        private DateTime m_dateMax = new DateTime(2222, 1, 1);
        ...
        private bool m_dateRecalcQueued;
        ...
        public UbsPmTradeFrm()
        {
            ...
            this.txtBIK0.Leave += new System.EventHandler(this.txtBIK0_Leave);
            this.txtBIK1.Leave += new System.EventHandler(this.txtBIK1_Leave);

            this.txtBIK0.Text = string.Empty;
        ...
        {
            mc["ID_TRADE"] = 0;
            mc["DATE_TRADE"] = m_dateMax;
            mc["NUM_TRADE"] = string.Empty;
            mc["IS_COMPOSIT"] = 0;
            mc["TYPE_TRADE"] = -1;
            mc["Вид сделки"] = -1;
            mc["Инструкция по поставке"] = 0;
            mc["Идентификатор валюты оплаты"] = 0;
            mc["Идентификатор драг.металла"] = -1;
            mc["Вид поставки по сделке"] = -1;
            mc["Идентификатор комиссии"] = -1;
        }
        ...
        private object ListKey(object param_in, ref object param_out)
        {
            try
            {
                ...
                if (m_command == CmdDel)
                {
                    this.IUbsChannel.LoadResource = DelLoadResource;
                    if (!isRecordsExist)
                    {
                    ...
                    {
                        base.IUbsChannel.ParamIn("IdTrades", param_in);
                        base.IUbsChannel.Run("PmDelTradeSrvByParam");
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            try
            {
                // Снимаем отложенный пересчёт по дате, чтобы он не выстрелил уже
                // после закрытия формы (форма к тому моменту может быть disposed).
                m_dateRecalcQueued = false;
                this.Close();
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                // Снимаем отложенный пересчёт по дате: ниже всё равно вызывается
                // CheckDatesOblig(), а серверные запросы курсов нужны только для
                // отображения и не влияют на отправляемые на сервер данные
                // (в FillArrOblig() ставка берётся из lvwObligation, а не из
                // ucdRateCurOblig/ucdCostUnit). Иначе отложенный BeginInvoke мог бы
                // повторно показать те же сообщения уже во вложенном цикле сообщений
                // (например, когда сейчас откроется MessageBox по результату сохранения).
                m_dateRecalcQueued = false;
                ...
                if (string.Equals(m_command, CmdEdit, StringComparison.Ordinal))
                    ...
                    base.UbsChannel_ParamIn("ID_TRADE", 0);

                if (tabControlInstr.TabPages.Contains(tabPageInstr1))
                    base.UbsChannel_ParamIn("Инструкция по оплате для покупателя", FillArrDataInstr(0));
                if (tabControlInstr.TabPages.Contains(tabPageInstr2))
                    base.UbsChannel_ParamIn("Инструкция по оплате для продавца", FillArrDataInstr(1));

                base.UbsChannel_ParamIn("IsNDS", chkNDS.Checked ? 1 : 0);
                base.UbsChannel_ParamIn("IsExport", chkExport.Checked ? 1 : 0);
                base.UbsChannel_ParamIn("IsExternalStorage", chkExternalStorage.Checked ? 1 : 0);
                ...
                if (IsMainDataChanged())
                {
                    ...
                    base.UbsChannel_ParamIn("Основные данные", objParamTrade);
                    blnRun = true;
                }

                if (m_needSendOblig)
                {
                    base.UbsChannel_ParamIn("ID_CLIENT1", m_idClient1);
                    base.UbsChannel_ParamIn("ID_CLIENT2", m_idClient2);

                    object[,] arrOblig = FillArrOblig();
                    base.UbsChannel_ParamIn("Обязательства сделки", arrOblig);
                    base.UbsChannel_ParamIn("Обязательства сделки2", m_paramOblig);
                    blnRun = true;
                }
                ...
                if (blnRun)
                {
                    base.UbsChannel_Run("ModifyTrade");
        ...
        private void dateTrade_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                // Если дата фактически не менялась с момента последней синхронизации
                // m_mc — никакой работы делать не нужно. Это в т.ч. защищает от
                // ситуации, когда пользователь сразу после открытия формы на
                // редактирование жмёт «Сохранить»: фокус уходит с dateTrade, но
                // нам нет смысла перезапрашивать курсы и переписывать обязательства.
                object prev;
                bool dateUnchanged =
                    m_mc.TryGetValue("DATE_TRADE", out prev)
                    && prev is DateTime
                    && (DateTime)prev == dateTrade.DateValue;

                if (dateUnchanged) return;

                // Минимальная синхронная часть, нужная для корректного сохранения:
                // обновить m_mc и пометить обязательства к отправке. CheckDatesOblig
                // в любом случае ещё раз отработает в btnSave_Click перед сохранением,
                // поэтому здесь его дублировать не обязательно.
                m_mc["DATE_TRADE"] = dateTrade.DateValue;
                m_needSendOblig = true;

                // Тяжёлая часть (валидация дат обязательств, переписывание подэлементов
                // ListView и серверные запросы курсов GetRate_CB / GetRateForPM)
                // откладывается на следующий проход сообщений Windows. Иначе
                // выполнение этих синхронных вызовов происходит между WM_LBUTTONDOWN
                // и WM_LBUTTONUP на btnSave/btnExit, и сам Click по кнопке теряется —
                // пользователю приходится кликать «Сохранить» дважды.
                if (!m_dateRecalcQueued && this.IsHandleCreated)
                {
                    m_dateRecalcQueued = true;
                    this.BeginInvoke((MethodInvoker)delegate
                    {
                        if (!m_dateRecalcQueued) return;
                        m_dateRecalcQueued = false;
                        if (this.IsDisposed) return;
                        try { ProcessDateTradeChange(); }
                        catch (Exception ex) { this.Ubs_ShowError(ex); }
                    });
                }
            }
        ...
        /// <summary>
        /// Тяжёлая часть обработки изменения даты сделки — валидация дат
        /// обязательств и серверные запросы курсов. Вызывается отложенно из
        /// <see cref="dateTrade_Leave"/> через BeginInvoke, чтобы не блокировать
        /// клик по кнопкам формы.
        /// </summary>
        private void ProcessDateTradeChange()
        {
            if (!CheckDatesOblig()) return;
        ...
        private void cmbUnit_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                GetMassaPrecision();
                ucdMass.DecimalValue = 0;
                ucdMassGramm.DecimalValue = 0;
        ...
        private void InitDoc()
        {
            try
            {
                ...
                if (isEdit)
                {
                    ...
                    {
                        var oblig = obligOut.Value("Обязательства сделки2") as UbsParam;
                        m_paramOblig.Items = oblig.Items;
                    }
                ...
                var args = new TabControlCancelEventArgs(tabPage1,
                                                         tabControl.TabPages.IndexOf(tabPage1),
                                                         false,
                                                         TabControlAction.Selecting);

                tabControl_Selecting(tabControl, args);
        ...
        {
            base.IUbsChannel.ParamIn("IsExternalStorage", isExternalStorage);
        ...
        private void tabControl_Selecting(object sender, TabControlCancelEventArgs e)
        {
            ...
            try
            {
                ...
                if (hasK1 && hasK2 && k1 == k2 && target != tabPage1)
                {
        ...
        private void CallOblig(string type)
        {
            ...
            if (string.Equals(type, PrefixAddOblig, StringComparison.Ordinal))
            {
                ...
                ucdCostUnit.DecimalValue = 0;
                ucdMass.DecimalValue = 0;
                ucdMassGramm.DecimalValue = 0;
                ucdSumObligation.DecimalValue = 0;
                ucdSumPayment.DecimalValue = 0;
            ...
            else
            {
                ...
                ucdCostUnit.DecimalValue = Convert.ToDecimal(sel.SubItems[4].Text);
                ucdMass.DecimalValue = Convert.ToDecimal(sel.SubItems[5].Text);
                GetMassGramm();
                ucdSumObligation.DecimalValue = Convert.ToDecimal(sel.SubItems[6].Text);

                int curId;
                if (int.TryParse(sel.SubItems[7].Text, out curId))
                    UbsPmTradeComboUtil.SetComboByKey(cmbCurrencyObligation, curId);

                ucdRateCurOblig.DecimalValue = Convert.ToDecimal(sel.SubItems[8].Text);
                ...
                ucdCostCurPayment.DecimalValue = Math.Round(ucdCostUnit.DecimalValue * ucdRateCurOblig.DecimalValue, 4);

                ucdSumPayment.DecimalValue = Math.Round(ucdSumObligation.DecimalValue * ucdRateCurOblig.DecimalValue, 2);
        ...
        private void FillDataObject(int idObject)
        {
            ...
            if (!po.Contains("Данные объекта"))
                return;

            Array arr = po.Value("Данные объекта") as Array;
            if (arr == null || arr.Rank != 2)
                return;
            ...
            decimal val;
            decimal currencyLigMassa;
            decimal currencyProba;
            if (decimal.TryParse(item.SubItems[1].Text, out val))
            {
                item.SubItems[1].Text = val.ToString("N2");
                currencyLigMassa = val;
            }
            else
            {
                currencyLigMassa = 0m;
            ...
            // проба
            if (decimal.TryParse(item.SubItems[2].Text, out val))
            {
                item.SubItems[2].Text = val.ToString("N2");
                currencyProba = val;
            }
            else
            {
            ...
            if (m_blnConvert)
            {
                if (cmbUnit.Text == "грамм")
                    currencyLigMassa = (m_strCB == "A99")
                        ? Math.Round(currencyLigMassa * currencyProba / 100m, 0)
                        : Math.Round(currencyLigMassa * currencyProba / 100m, 1);
                else
                    currencyLigMassa = Math.Round(currencyLigMassa * currencyProba / 100m, 2);
            }

            ucdMass.DecimalValue = (ucdMass.DecimalValue + currencyLigMassa);
        ...
        private void FillArrObject(out object[,] objectArray)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (item.SubItems.Count > 5)
                    int.TryParse(item.SubItems[5].Text, out idObject);
        ...
        {
            lblObligationInfo2.Text = $"Цена: {ucdCostUnit.DecimalValue} Масса: {ucdMass.DecimalValue} Сумма: {ucdSumPayment.DecimalValue}";
        }
        ...
        private bool CheckKey(int g)
        {
            ...
            if (bik.Length > 0 && rs != EmptyAccountNumber)
            {
        ...
        private bool CheckDatesOblig()
        {
            ...
            if (tabControlInstr.TabPages.Contains(tabPageInstr1) && ctKey2 != 0)
            {
                ...
                if (strRS.Length >= 8)
                {
                    string strIdCurrencyRS = strRS.Substring(5, 3);
                    if (txtBIK0.Text.Trim() == m_ourBIK)
                    {
                        if (!IsEqualNumIdCurr(paymentCurKey.ToString(), strIdCurrencyRS))
                        {
                    ...
                    else if (strRS != EmptyAccountNumber)
                    {
                        if (!IsEqualNumIdCurr(paymentCurKey.ToString(), strIdCurrencyRS))
                        {
                            MessageBox.Show(this, MsgBuyerRsCurrencyMismatch, MsgTitleValidationProps,
            ...
            if (tabControlInstr.TabPages.Contains(tabPageInstr2) && ctKey1 != 0)
            {
                ...
                if (strRS.Length >= 8)
                {
                    string strIdRS1 = strRS.Substring(5, 3);
                    if (txtBIK1.Text.Trim() == m_ourBIK)
                    {
                        if (!IsEqualNumIdCurr(paymentCurKey.ToString(), strIdRS1))
                        {
        ...
        private static bool IsEqualNumIdCurr(string code1, string code2)
        {
            int n1, n2;
        ...
        private void SnapshotMc()
        {
            ...
            if (m_command != CmdEdit)
            {
                FulFillMainCollection(m_initialMc);
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (ids != null && ids.Length > 0)
            {
                ...
                {
                    foreach (KeyValuePair<int, string> item in cmbComission.Items)
                    {
        ...
        private void linkLabel2_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (cmbContractType2.SelectedIndex == 1)
            {
                filterAction = UBS_PM_BROKER_LIST;
            }
            else if (cmbContractType2.SelectedIndex == 2)
            {
                filterAction = UBS_PM_CONTRACT_LIST;
            }
        ...
        private void linkListInstr0_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                ...
                ShowInstrDialog(0, base.IUbsChannel.ParamsOutParam);
            }
            catch (Exception ex) { this.Ubs_ShowError(ex); }
        }
        ...
        private void linkListInstr1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                ...
                ShowInstrDialog(1, base.IUbsChannel.ParamsOutParam);
            }
            catch (Exception ex) { this.Ubs_ShowError(ex); }
        }

        private void ShowInstrDialog(int index, UbsParam paramsOutParam)
        {
            using (FrmInstr frm = new FrmInstr())
            {
                if (paramsOutParam.Contains("ArrInstr"))
                    frm.LoadData(paramsOutParam.Value("ArrInstr"));

                if (frm.ShowDialog(this) != DialogResult.OK)
                    return;

                object[] arr = frm.SelectedInstr;
                if (arr == null)
                    return;

                if (index == 0)
                {
                    txtBIK0.Text = Convert.ToString(arr[0]);
                    txtName0.Text = Convert.ToString(arr[1]);
                    ucaKS0.Text = Convert.ToString(arr[2]);
                    ucaRS0.Text = Convert.ToString(arr[3]);
                    txtClient0.Text = Convert.ToString(arr[4]);
                    txtNote0.Text = Convert.ToString(arr[5]);
                    txtINN0.Text = Convert.ToString(arr[6]);
                    chkNotAkcept0.Checked = Convert.ToInt32(arr[7]) != 0;
                }
                else
                {
                    txtBIK1.Text = Convert.ToString(arr[0]);
                    txtName1.Text = Convert.ToString(arr[1]);
                    ucaKS1.Text = Convert.ToString(arr[2]);
                    ucaRS1.Text = Convert.ToString(arr[3]);
                    txtClient1.Text = Convert.ToString(arr[4]);
                    txtNote1.Text = Convert.ToString(arr[5]);
                    txtINN1.Text = Convert.ToString(arr[6]);
                    chkNotAkcept1.Checked = Convert.ToInt32(arr[7]) != 0;
                }

                string bik = Convert.ToString(arr[0]);
                if (string.Equals(bik, m_ourBIK, StringComparison.Ordinal))
                {
                    if (index == 0) { ucaRS0.ReadOnly = true; linkAccountPayment0.Enabled = true; linkAccountPayment0.Focus(); }
                    else { ucaRS1.ReadOnly = true; linkAccountPayment1.Enabled = true; linkAccountPayment1.Focus(); }
                }
                else
                {
                    if (index == 0) { ucaRS0.ReadOnly = false; linkAccountPayment0.Enabled = false; ucaRS0.Focus(); }
                    else { ucaRS1.ReadOnly = false; linkAccountPayment1.Enabled = false; ucaRS1.Focus(); }
                }
            }
        }
        ...
        private void linkAccountPayment0_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                InitAccountPaymentByPart(0);
            }
            catch (Exception ex) { this.Ubs_ShowError(ex); }
        }

        private void InitAccountPaymentByPart(int part)
        {
            object[] ids = this.Ubs_ActionRun(ACCOUNT0_FLT, this, null) as object[];
            if (ids != null && ids.Length > 0)
            {
                var uca = part == 0 ? ucaRS0 : ucaRS1;

                uca.Text = GetAccountPayment(Convert.ToInt32(ids[0]));
            }
        }

        private string GetAccountPayment(int idAccount)
        {
            base.IUbsChannel.ParamIn("IdAccount", idAccount);
            base.IUbsChannel.Run("TradeGetAccountPM");

            return Convert.ToString(base.IUbsChannel.ParamOut("Account Name"));
        }
        ...
        private void cmdDelObject_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    ucdMass.DecimalValue = (ucdMass.DecimalValue - adjustedMass);
                }
        ...
        private void tabControlOblig_Selecting(object sender, TabControlCancelEventArgs e)
        {
            try
            {
                ...
                if (target == tabPageOblig2) // переход на вкладку "Объекты"
                {
                    ...
                    lblObligationInfo1.Text = $"Дата оплаты: {datePayment.DateValue.ToShortDateString()} Дата поставки: {datePost.DateValue.ToShortDateString()}";
        ...
        private void txtBIK0_Leave(object sender, EventArgs e)
        {
            if (txtBIK0.TextLength < 9)
            {
                return;
            }
        ...
        private void txtBIK1_Leave(object sender, EventArgs e)
        {
            if (txtBIK1.TextLength < 9)
            {
                return;
            }
